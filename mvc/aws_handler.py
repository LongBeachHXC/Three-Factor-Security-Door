...
class AwsHandler():
    ...
    def comparePhotos(self, user_db_photo, snapshot_photo):
        source_image = requests.request('GET', user_db_photo)
        target_image = requests.request('GET', snapshot_photo)
        
        response=self.rek_client.compare_faces(SimilarityThreshold=80,
                                  SourceImage={'Bytes': source_image.content},
                                  TargetImage={'Bytes': target_image.content}
                                )
